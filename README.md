...
从`CQA`的本质来看，主要是[cls] c [sep] q [sep]->a

那么实际上是c的embedding和q的embedding中关于数值的encode 向量并没有拉开较大的距离

对于15.3% 和15.7%来说，它们的embedding是很相似的

一是需要对tokenizer的过程再看看分词的结果

二是考虑SQuAD学习no answer的方法，对于样本来说，某个数值，扰动后的数值'，->no answer

三是直接在训练集中加入很多"伪数值"，做数据增强，提高数据难度
...
1) knowledge distillation（KD过程）
   ...
   模型参数由24层缩小到12层，推理速度快了将近一倍，由于训练数据足够多，精度损失基本不大。
